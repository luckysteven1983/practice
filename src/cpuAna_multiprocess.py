...
#         dictSecond[i] = []
    ...
    def __init__(self, lab):
        ...
        dictSecond = {}
        dictLab = {}
        dictLab[lab] = [dictName, dictTime, dictSheet, dictSecond]
        self.dictLab = dictLab
        for i in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L']:
            dictName[i] = {}
            dictTime[i] = []
            dictSecond[i] = []
        self.dictName = self.dictLab[lab][0]
        self.dictTime = self.dictLab[lab][1]
        self.dictSheet = self.dictLab[lab][2]
        self.dictSecond = self.dictLab[lab][3]
...
#             self.writeToSheet(workbook, 'Sta_'+self.fileName, result)
        ...
        for i in loop:
            threads2[i].join()
        print '%s: cpuAna parsing DONE!'%datetime.datetime.now()
        ...
        for j in loop:
            threads1[j].join()
        print '%s: writeToSheet parsing DONE!'%datetime.datetime.now()
...
if __name__=='__main__':
    def parseArg():
        if len(sys.argv) != 5:
            ...
            return startTime, endTime, labList, directory
    arg = parseArg()
    print '---test cpuAna_multiprocess.py starts!---\n'\
          '---parsing data from startTime: '+arg[0]+' to endTime: '+arg[1]+'---'           
    print datetime.datetime.now()
    labList = arg[2].split(',')
    loop = range(len(labList))
    threads3 = []
    for i in loop:
        #fileFolder = '/PLATsoftware/pt_result/2015-12-03/fm1945_'+lab+'/Done/top_*.log'
        lab = labList[i]
        lab = lab.strip()
        cpuAnaAManager = CPUAnaAManager(lab)
        #print '%s: Start to get sshconn from lab %s'%(datetime.datetime.now(), lab)
        ...
        t3 = threading.Thread(target=cpuAnaAManager.cptAnaAll, args=(arg[0], arg[1], lab, './cache/'+lab))
