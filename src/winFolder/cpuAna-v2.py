...
#    ssh = ssh_manager.SshManager()
    ...
    def cpuAna(self, startTime, endTime, fileInput, fileName):
        ...
        print fileName, len(self.dictName[fileName])
    ...
    def writeToSheet(self, workbook, sheetName, dictCpu, name):
...
#             self.writeToSheet(workbook, 'Sta_'+self.fileName, result)
        ...
        for j in loop:
            ...
            self.writeToSheet(workbook, 'Sta_'+name, self.dictName[name], name)
            #sheetname1 = workbook.add_worksheet('Sta_'+name)
            #chart = workbook.add_chart({'type': 'line', 'name': 'Sta_'+name+'_cpu'})

                   
        print 'A', self.dictTime['A'][-30:-1]
